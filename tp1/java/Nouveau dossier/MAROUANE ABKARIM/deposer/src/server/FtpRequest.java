...
public class FtpRequest extends Thread {
    ...
    /*fonction pour faire l'initialisation de notre socket 
    ainsi d'un objet auth qui nous permet de faire la verification des utilisateur avec leur password
    ,et le repertoir qui est définie par l'admi a notre client */
...
/* focntion qui vérifier si l'utilisateur exsite*/
...
/* vérifier si le password d'un utlisateur et confirme le login saisie*/
...
/* cleint demand de quitter le serveur */
...
/* reépérer l'adresse et numero de port pour faire une connexion de donnée en mode actif*/
...
/* preparer pour envoyer l'adresse et numero de port au client pour faire une connexion data mode passif*/
...
/* list le contenu de repertoir courant distant */
    public void processLIST() throws NotAuthentificationException, IOException {
        ...
        if (isConnected()) {
            ...
            /* récupérer la liste des fichier/répértoie existe dans un chemin ( repertoire cournat distant) */
            File[] files = new File(toolServer.getCurrentDirectory()).listFiles();
            for (File file : files) {
                /*récupérer le nom de fichier existe dans le repoirtoire courant distant*/
                FileName = file.getName();
                /*recupérer la date de la dernier mise à jour */
                date = new Date(file.lastModified());
                /*recuperé le repertoir et nom user */
    ...
    public void processCWD(String chemin) throws IOException, NotAuthentificationException {
        ...
        if (!this.isConnected()) {
            ...
            if (chemin.indexOf("..") == -1) {
                if(chemin.equals("//")){
                    /*appel de cette focntion ,vas nour retournée au repertoir parent */
                    processCDUP();
                }else{
...
/* créer un repertoir dans le repertoir courant */
...
/*supprimer un fichier/repertoir de reperoir courant*/
...
/*renomer un fichier/repertoire existe dans le repertoire courant */
...
/* la verification de l'existance de fichier qu'on veuxx renomer par la fonction proessRNFR 
    aprés de l'existance de fichier le renomage et fait par la fonction processRNTO */
